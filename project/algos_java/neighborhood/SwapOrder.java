...
public class SwapOrder extends Move {

    private int addedOrder = -1;
    private int removedOrder = -1;
    private Solution savedState;

    public SwapOrder(Problem problem, Random random, String name) {
    ...
    public double doMove(Solution solution) {
        ...
        if (solution.orders.isEmpty() || solution.orders.size() == problem.nOrders) {
            return 0;
        }
        ...
        // Temporarily add removedOrder stock back
        for (Map.Entry<Integer, Integer> entry : problem.orders.get(removedOrder).entrySet()) {
            remainingStock[entry.getKey()] += entry.getValue();
        }

        List<Integer> availableOrders = new ArrayList<>();
        int currentTotalItems = solution.getTotalItemsPicked() - problem.orderUnits[removedOrder];

        for (int i = 0; i < problem.nOrders; i++) {
            if (!solution.orders.contains(i) && i != removedOrder) {
                ...
                if (canFulfill && currentTotalItems + problem.orderUnits[i] <= problem.ub) {
    ...
    public boolean hasMove(Solution solution) {
        return solution.aisles.size() > 0 && solution.orders.size() > 0 && solution.orders.size() < problem.nOrders;
    }
